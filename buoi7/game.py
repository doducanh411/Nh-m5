...
# Ô tô đối thủ
car_image = pygame.image.load('oto1.png')  # Đường dẫn tới hình ảnh ô tô của bạn
car_image = pygame.transform.scale(car_image, (40, 80))  # Thay đổi kích thước nếu cần
car_speed = 2  # Ô tô ban đầu di chuyển chậm hơn
car_increase_interval = 2 * FPS  # Thêm ô tô mỗi 2 giây
car_speed_increase_interval = 5 * FPS  # Tăng tốc ô tô mỗi 5 giây
car_spawn_interval = 5 * FPS  # Tăng số lượng ô tô mỗi 5 giây
car_increase_timer = 0
car_speed_timer = 0  # Thời gian để tăng tốc ô tô
car_spawn_timer = 0  # Thời gian để tăng số lượng ô tô
...
while True:
    ...
    car_increase_timer += 1
    if car_increase_timer >= car_increase_interval:
        car_increase_timer = 0
        cars.append(tao_xe())  # Thêm ô tô mới vào danh sách

    # Tăng tốc ô tô mỗi 5 giây
    car_speed_timer += 1
    if car_speed_timer >= car_speed_increase_interval:
        car_speed_timer = 0
        car_speed += 1  # Tăng tốc độ ô tô mỗi 5 giây

    # Tăng số lượng ô tô mỗi 5 giây (tối đa 6 ô tô)
    car_spawn_timer += 1
    if car_spawn_timer >= car_spawn_interval and len(cars) < 6:
        car_spawn_timer = 0
        cars.append(tao_xe())  # Thêm ô tô mới vào danh sách
    ...
    # Vẽ ô tô từ hình ảnh
    for car, _ in cars:
        w.blit(car_image, car.topleft)  # Vẽ ô tô tại vị trí của hình chữ nhật ô tô
